...
void Apropos::setApropos(string config, string Theme, string gui, string lg){
   ...
   adr.clear();
   adr.str("");
   adr << std::fixed << fGui<<"metaFx2.png";
   ...
   string file=config+"/Lang/"+lg;
   std::cout << "gui apropos" <<file<< std::endl;
   ...
   if(fichier){ 
     ...
     fichier.close();		
       ...
       cerr << "Erreur à l'ouverture de Lang!" << endl;
  }
  	string s;
   s=config+"/Theme/"+Theme+"/"+Theme;
   std::cout << "th apropos" <<s<< std::endl;
   ifstream fichier2(s, ios::in); 
   contenu="";
...
void Apropos::onClick(sf::Event e, string nav){
   ...
   if(e.mouseButton.x>182 && e.mouseButton.x<236 && e.mouseButton.y>394 && e.mouseButton.y<406){
	  string cmd=nav+" http://blanchemain.info/Documents/Programmation/index.php?page=metaFx &";
