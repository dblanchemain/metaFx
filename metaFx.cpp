...
//*********************************************************************************************************************  
   ...
   winPrincipale.draw(menuAide);
	...
	for(int i=0;i<tableMixer.size();i++){
		...
		canalTextF1.setFillColor(genTcolor[tableMixer[i].getGroupe()]);
		ctxt=to_string(tableMixer[i].getTrack());
		if(tableMixer[i].getTrack()!=0){
			canalTextF1.setString(ctxt);
	       if(tableMixer[i].getTrack()<10){
			 	canalTextF1.setPosition(sf::Vector2f(tableMixer[i].getPosX()+8, tableMixer[i].getPosY()+5));
			 }else{
			 	canalTextF1.setPosition(sf::Vector2f(tableMixer[i].getPosX()+5, tableMixer[i].getPosY()+5));
			 }
			 winPrincipale.draw(canalTextF1);
		}
	}  
	 if(mixerActif!=65535){
	 	selectMixer.setPosition(sf::Vector2f(tableMixer[mixerActif].getPosX(),tableMixer[mixerActif].getPosY()));
...
void onClickPave(sf::Event e){
   ...
   if(tableMixer[mixerActif].getFx()==0 && e.mouseButton.x>244 && e.mouseButton.x<274  && e.mouseButton.y>236 && e.mouseButton.y<268){
   	string name="Graphe"+to_string(mixerActif);
		...
		winSecond[mixerActif].create(sf::VideoMode(1200,600), name);
...
void controlSpeedMixer(int id){
	...
	float mx;
	string cm;
	if(tableMixer[id].getSpeed()==0){
		mx=(tableMixer[id].getSpeedMax()-tableMixer[id].getSpeedMin())/dx;
		dt2=distance(windowAppWidth/2,windowAppHeight/2,tableMixer[id].getPosX(),tableMixer[id].getPosY());
		if(tableMixer[id].getRevSpeed()==1){
			dt=tableMixer[id].getSpeedMax()-(dt2*mx);
		}else{
			dt=tableMixer[id].getSpeedMin()+(dt2*mx);	
		}
		if(dt<tableMixer[id].getSpeedMin()){
			dt=tableMixer[id].getSpeedMin();
		}
		if(dt>tableMixer[id].getSpeedMax()){
			dt=tableMixer[id].getSpeedMax();
		}
		cm="/fileFx/Mixer/M"+to_string(id)+"/Player/Param/speed_"+to_string(id);
		oscSendGain(cm,dt);
	}
}
...
void saveDsp(string dsp, string param){
   ...
   fichier.close();
   string cmd=defInstall+"/dynamic-jack-gtk -llvm -osc "+wfile+" &";
   char* dest = new char[cmd.length() + 1];
   std::copy(cmd.begin(), cmd.end(), dest);
   int id=system(dest);
   sleep (1);
...
void onMouseWheel(sf::Event e){
	int nposy;
  if(e.mouseWheelScroll.x>4 && e.mouseWheelScroll.x<100 && e.mouseWheelScroll.y>30 && e.mouseWheelScroll.y<250){
	  if (e.mouseWheelScroll.wheel == sf::Mouse::VerticalWheel){
		if(e.mouseWheelScroll.delta==1){
			baseFxIndex=baseFxIndex-1;
			if(baseFxIndex<1){
			 	baseFxIndex=0;
			 	nposy=0;
			}else{
			...
			baseFxIndex=baseFxIndex+1;
			if(baseFxIndex>tableLabelModule.size()-12){
			 	baseFxIndex=tableLabelModule.size()-12;
			 	nposy=0;
			}else{
				nposy=1;
			}
	  ...
	  nposy=menuFxSlider.getPosition().y+nposy;
	  if(nposy<menuFxBkgSlider.getPosition().y){
	  		nposy=nposy<menuFxBkgSlider.getPosition().y;
	  }
...
void moduleDelete(int id, int dest){
	...
	if(tableMixer[id].listModules[dest].label!="Entree" && tableMixer[id].listModules[dest].label!="Sortie"){
		...
		tableMixer[id].listModules[dest].type=1;
...
void connectorDel(int id, int dest){
	...
	tableMixer[id].tableConnectors[dest].type=1;
