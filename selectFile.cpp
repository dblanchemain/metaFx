...
void selectFile::setSelectorWidth(int w){
	winSelectorWidth=w;
}
void selectFile::setSelectorHeight(int h){
	winSelectorHeight=h;
}
...
void selectFile::setTitreSelector(sf::String sT){
    titreSelector=sf::String::fromUtf8(sT.begin(),sT.end());
}
...
int selectFile::getSelectorWidth(){
	return winSelectorWidth;
}
...
void selectFile::initSelector(string fdir){
  ...
  adr.clear();
  adr.str("");
  winSelector.setTitle(L"Selecteur de fichier");
  nText.setString("");
  dossier=fdir;
  dossierPath.setString(dossier);
  listDirs(dossier);
  readFiles();
  std::cout << "fileselector theme: " <<fTheme<<std::endl;
  std::cout << "fileselector gui: " <<fGui<<std::endl;
